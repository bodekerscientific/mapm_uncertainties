...
# ODIN RAW colocation 1 #######
file_in_path <- "./data/ODIN/Colocation_1/Raw/NetCDF/"
nc_files <- dir(file_in_path,full.names = FALSE,pattern = "nc")

# Create bucket
bucket_name <- "mapm-odin-raw-colo1"
...
# ODIN RAW colocation 2 #######
file_in_path <- "./data/ODIN/Colocation_2/Raw/NetCDF/"
nc_files <- dir(file_in_path,full.names = FALSE,pattern = "nc")

# Create bucket
bucket_name <- "mapm-odin-raw-colo2"
if (!bucket_exists(bucket_name)[1]){
  put_bucket(bucket_name)
}
for (nc_file in nc_files) {
...
# ODIN NN colocation 1 #######
file_in_path <- "./data/ODIN/ODIN_NN/Colocation_1/"
nc_files <- dir(file_in_path,full.names = FALSE,pattern = "nc")

# Create bucket
bucket_name <- "mapm-odin-nn-colo1"
if (!bucket_exists(bucket_name)[1]){
  put_bucket(bucket_name)
}
for (nc_file in nc_files) {
...
# ODIN NN colocation 2 #######
file_in_path <- "./data/ODIN/ODIN_NN/Colocation_2/"
nc_files <- dir(file_in_path,full.names = FALSE,pattern = "nc")

# Create bucket
bucket_name <- "mapm-odin-nn-colo2"
if (!bucket_exists(bucket_name)[1]){
  put_bucket(bucket_name)
}
for (nc_file in nc_files) {
...
# ODIN v01 colocation 1 #######
file_in_path <- "./data/ODIN/Colocation_1/Raw/NetCDF/"
file_out_path <- "./data/ODIN/Colocation_1/v01/NetCDF/"
...
file.copy(paste0(file_in_path,nc_files_in),
          ...
          overwrite = TRUE)
v01.correction <- read_delim("./data/v01_ODIN_coefficients.txt",
                             delim = "\t",
                             col_names = TRUE)
...
# Create bucket
bucket_name <- "mapm-odin-v01-colo1"
if (!bucket_exists(bucket_name)[1]){
  put_bucket(bucket_name)
}
for (nc_file in nc_files) {
  # Get Device to match correction coefficients
  curr_dev <- paste0("ODIN",substr(nc_file,6,11))
...
file_in_path <- "./data/ODIN/Colocation_2/Raw/NetCDF/"
file_out_path <- "./data/ODIN/Colocation_2/v01/NetCDF/"
nc_files_in <- dir(file_in_path,full.names = FALSE,pattern = "nc")
nc_files <- stringr::str_replace(nc_files_in,"raw","v01")
file.copy(paste0(file_in_path,nc_files_in),
          paste0(file_out_path,nc_files),
...
# Create bucket
bucket_name <- "mapm-odin-v01-colo2"
if (!bucket_exists(bucket_name)[1]){
  put_bucket(bucket_name)
}
for (nc_file in nc_files) {
...
# ODIN RAW deployment #######
file_in_path <- "./data/ODIN/Deployment/Raw/NetCDF/"
nc_files <- dir(file_in_path,full.names = FALSE,pattern = "nc")

# Create bucket
bucket_name <- "mapm-odin-raw-deployment"
if (!bucket_exists(bucket_name)[1]){
  put_bucket(bucket_name)
}
for (nc_file in nc_files) {
...
# ODIN v01 deployment #######
file_in_path <- "./data/ODIN/Deployment/v_01/NetCDF/"
nc_files <- dir(file_in_path,full.names = FALSE,pattern = "nc")

# Create bucket
bucket_name <- "mapm-odin-v01-deployment"
if (!bucket_exists(bucket_name)[1]){
  put_bucket(bucket_name)
}
for (nc_file in nc_files) {
